...
A collaboration Spawn batch additionally commits its `task_created` records and
a complete Team/membership batch marker in that same ledger transaction; only
after the durable commit succeeds are Task, Team, and membership projections
published. Exact replay reuses the identical marker. Conversation, fork, and
configured-Supervisor paths first atomically prepare an exact Team shell before
the child identity can be reserved, then atomically add the child membership
before the conversation is exposed as usable. A failed membership commit aborts
the reserved child and opening claim; an exact retry can reuse the durable Team
shell without duplicating the full batch. A ledger backend that cannot guarantee
these metadata commits rejects before the corresponding projection changes.
Cold recovery accepts an ownerless Team Task only while it is still `Queued`,
and only when the Team and its coordinator membership are already durable; all
other Team Tasks require a durable member owner.

Workflow retry policy is type-based. Only an explicit `Retryable` failure class
authorizes automatic retry. A failed child Workflow stores a deterministic
summary of every failed branch; the parent is Retryable only when every member
of that set is `Retryable`. `ReconciliationRequired`, `SideEffectUnknown`, and
`OutcomeUnknown` have safety priority when choosing the primary failure, so a
retryable sibling cannot hide an outcome that must be reconciled. The summary
is part of the durable Workflow settlement and is recomputed/verified on
restore. A bare `Failed` result is `NonRetryable`, and permission, cancellation,
convergence, turn-budget, unknown side-effect, unknown outcome, and
reconciliation failures retain their typed terminal or recovery semantics.
...
The Windows Full runner uses the packaged helper to create an AppContainer target with no network capabilities. Temporary ACL leases grant only the launch profile access to the workspace and private directories, keep Runtime state unavailable, and remain correct for concurrent launches in one Solaris process. The outer helper and all descendants remain in a kill-on-close Job Object, which is terminated immediately when the sandbox target completes so background descendants cannot outlive the operation. A real probe launches and waits for an AppContainer target before Full is cached; if the current account cannot initialize that child, the probe returns Partial or Unavailable before the requested target starts. The current native Windows suite executes 41 top-level tests successfully under `NT SERVICE\WebCodexRunner`. A fresh interactive `Administrator` Session 1 run also executed the real approved-domain test path and received the same typed `NetworkProxyUnavailable` result before the requested Target could be returned; interactive and service identities therefore both have current fail-closed evidence on this host.
...
Auto child processes have no direct network access. On Linux and macOS, a process may reach an exact approved HTTP(S) host and port only through the per-launch Host proxy. The proxy rejects IP literals, wildcards, user information, localhost/metadata names, private or special-use DNS results, and malformed or oversized proxy requests; it resolves the name again before connecting. Windows uses the same exact host+port policy through an installed packaged AppContainer proxy peer. The release archive contains a verified MSIX; the installer attempts per-account registration and publishes the peer PFN/AUMID only after Windows reports an installed package whose proxy executable is byte-identical to the release binary. The Target itself receives no `internetClient` or `privateNetworkClientServer` capability. Solaris creates a real PSEC environment, encodes only the loopback proxy plus `allowed_appcontainer_peer` and no direct egress, passes it with `PROC_THREAD_ATTRIBUTE_SECURITY_ENVIRONMENT`, and keeps the packaged proxy in a kill-on-close Job. Before the requested Target is created, a proof child with the same AppContainer/PSEC attributes must reach an approved endpoint through the proxy, receive a 403 for an unapproved endpoint, and fail a direct TCP connection to the exact public address the proxy just reached. The Windows proxy resolver uses an absolute hostname and `GetAddrInfoExW` with per-connection cache bypass, validates every returned address as public, and connects only to that validated address set, so a later private/special DNS answer fails rather than reusing prior public state. Proxy death, package/PSEC setup failure, or failed isolation proof returns a typed `SandboxReport` with backend `WindowsAppContainer`, enforcement `Unavailable`, and reason `NetworkProxyUnavailable` before the requested Target starts; there is no Ambient fallback. On the currently validated host, `processmodel.dll` exposes none of the required V2 `CreateProcessSecurityEnvironment`, `QueryProcessSecurityEnvironmentSupport`, or `CloseProcessSecurityEnvironment` entry points, so approved-domain launches remain unavailable before the proof Target can run. The interactive session reports Windows 10.0.26200.0, while the installed `processmodel.dll` file is 10.0.26100.8737. The logged-off `NT SERVICE\\WebCodexRunner` account cannot register AppX packages (`0x80073D19`); the interactive account additionally rejects unsigned executable-package Publisher registration at manifest/deployment validation on this host. The unsigned package builder includes Windows' documented unsigned Publisher OID so supported hosts do not fail merely because the marker is missing. Exports, package construction, codec tests, or proxy startup alone never produce Full.
