...
The product contract requires a Full sandbox backend for process execution in strict Auto. A Partial or Unavailable backend denies that process operation without disabling Auto file tools, and it must never fall back to Ambient execution. Auto child processes never receive direct network access. Linux and macOS have an approved-domain Host proxy implementation that revalidates DNS and destination IPs; their native proxy tests remain release evidence. Windows rejects network-bearing Auto process launches until a verified process-isolated proxy runner exists. This rejection is emitted as `status: "denied"` with `metadata.sandbox_report` set to backend `windows_app_container`, enforcement `unavailable`, and reason `network_proxy_unavailable`; Hosts must preserve those typed fields rather than replace them with the output string.
