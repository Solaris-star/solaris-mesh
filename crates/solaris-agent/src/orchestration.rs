...
async fn execute_single(
    ...
    if let Some(hook_engine) = hooks {
        ...
        for msg in messages {
            tracing::info!(target: "solaris_agent", hook_message = %msg, "post-tool-use hook output");
        }
    }

    let duration_ms = start.elapsed().as_millis() as u64;
    tracing::info!(target: "solaris_agent", duration_ms, success = !result.is_error, "tool execution completed");
