...
impl EffectExecutionContext {
    ...
    pub(crate) fn record_tool_calls_once(
        &self,
        round_call_id: &str,
        calls: &[solaris_types::tool::ToolCallStat],
    ) -> Result<(), String> {
        ...
        for (scope, resources) in [
            ...
            let Some(resources) = resources else { continue };
            resources
                .record_tool_calls_once_checked(&round_identity, calls)
                .map_err(|error| format!("{scope} tool-call statistics persistence failed: {error}"))?;
        }
        Ok(())
    }

    /// Statistics scope for duplicate tool-call detection.
    ///
    /// Combines the durable task scope with the execution environment scope so
    /// that identical calls made by different tasks or under a different
    /// provider/plugin/tool/workflow configuration are never treated as
    /// duplicates of each other.
    pub(crate) fn tool_call_scope(&self, task_scope: &str) -> String {
        let environment = self.environment();
        let environment_digest = environment::tool_call_scope_environment_digest(&environment);
        format!("task:{task_scope}|env:{environment_digest}")
    }
