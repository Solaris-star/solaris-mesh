...
use serde_json::json;
use solaris_agent::confirm::ToolConfirmer;
...
use solaris_config::config::{Config, ProviderType, SessionConfig, ToolsConfig};
use solaris_config::hooks::HooksConfig;
use solaris_mcp::config::McpConfig;
use solaris_providers::create_provider;
...
fn openai_config(api_key: &str) -> Config {
    Config {
        ...
        tools: ToolsConfig {
            auto_approve: true,
            allow_list: vec![],
            skills: solaris_config::config::SkillsPermissionConfig::default(),
        },
        ...
        file_cache: solaris_config::file_cache::FileCacheConfig::default(),
...
impl solaris_tools::Tool for FixedOutputTool {
    fn name(&self) -> &str {
        &self.name
    }
