...
async fn tc_11_1_bare_mode_only_loads_add_dirs() {
    ...
    // Put a skill in add_dir's .solaris/skills/
    let add_skills_dir = add_tmp.path().join(".solaris").join("skills");
    ...
    assert_eq!(filesystem_skills[0].name, "add-skill");
...
async fn tc_11_5_empty_scenario_returns_empty_vec() {
    ...
    let tmp = TempDir::new().unwrap();
    // tmp exists but has no .solaris/skills
...
// matching TS skillRoot used for ${SOLARIS_SKILL_DIR} substitution.
// -----------------------------------------------------------------------
