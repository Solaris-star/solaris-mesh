...
async fn tc_4_4b_command_fail_with_output_returns_ok() {
    ...
    use solaris_config::shell::{ShellKind, default_shell};
...
async fn tc_windows_skill_shell_powershell_stdout_preserved() {
    let result = run("before !`Write-Output solaris_skill_stdout_probe` after")
        .await
        .unwrap();

    assert!(
        result.contains("solaris_skill_stdout_probe"),
